...
void Arbitre::initialisation()
{
    ...
    switch (((_numero_partie%2)? _player1 : _player2)) {
        case player::M_1:
            _joueur1 = std::make_shared<Joueur_MonteCarlo_ABBEL> ("MonteCarlo",true);
            break;
        case player::M_2:
            _joueur1 = std::make_shared<Joueur_MonteCarlo_ABBEL> ("MonteCarlo2",true);
            break;
        case player::MANUEL:
    ...
    switch (((!(_numero_partie%2))? _player1 : _player2)) {
        case player::M_1:
            _joueur2 = std::make_shared<Joueur_MonteCarlo_ABBEL> ("MonteCarlo",false);
            break;
        case player::M_2:
            _joueur2 = std::make_shared<Joueur_MonteCarlo_ABBEL> ("MonteCarlo2",false);
            break;
        case player::MANUEL:
    ...
    for(int i=0 ; i < _nombre_parties ; i++)
        {
            ...
            std::this_thread::sleep_for (std::chrono::milliseconds(25)); // temps de latence entre deux parties
            _numero_partie++;
            initialisation();
        }
